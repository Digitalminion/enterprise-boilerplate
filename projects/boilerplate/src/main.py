...
class Main:
    def __init__(self):
        ...
        project = self.default_project.get('moduleName',{})
        module = __import__(project+'.module',fromlist=["*"])
        modulePrefix = self.default_project.get('modulePrefix',{})
        project_module_manifest = getattr(module, modulePrefix+'Module')
        # Read the module manidest to find the entry pont class
        component_class = project_module_manifest.get('bootstrap',{})
        component = __import__(project+'.component',fromlist=["*"])
        project_component_class = getattr(component, component_class[0])

        # # Initialize the entry pont class inject our application services of arg parse
        self.project_instance = project_component_class(self.args, self.log)
